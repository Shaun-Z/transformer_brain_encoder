...
def evaluate(model, criterion, data_loader, args,  lh_challenge_rois=None, rh_challenge_rois=None):
    ...
    for samples, targets in metric_logger.log_every(data_loader, 100, header):
        ...
        if (args.encoder_arch != 'linear') and (args.readout_res != 'hemis') and (args.readout_res != 'voxels'):
...
@torch.no_grad()
def test_batch(model, samples, readout_res, lh_challenge_rois, rh_challenge_rois):
    model.eval()

    lh_f_pred, rh_f_pred = evaluate_batch(model, samples, readout_res, lh_challenge_rois, rh_challenge_rois)
